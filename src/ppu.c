...
uint8_t color_palette[64][3] = {
    ...
    176, 90,  236, 228, 84,  236, 236, 88,  180, 236, 106, 100, 212, 136, 32,
...
static inline uint32_t getargbfrompal(ppu* PPU, uint8_t pixel,
                                      uint8_t palette) {
  uint8_t palent = pixel | (palette << 2);
  return (color_palette[PALETTE(palent)][2]) |
         (color_palette[PALETTE(palent)][1] << 8) |
         (color_palette[PALETTE(palent)][0] << 16) | (0xFF << 24);
}

static uint8_t next_tile_id, next_tile_attr, next_pattern_lsb, next_pattern_msb;
static uint16_t patternshiftreg1, patternshiftreg2, attribshiftreg1,
    attribshiftreg2, mux;
static uint16_t tile_addr, attribute_addr;
static uint8_t spritereg1[8], spritereg2[8], spritex[8], spriteattr[8], wait,
    OAMentry, n, m, zerosprflag;  // some variables to make handling the sprite
                                  // evaluation more accurate and easy
...
void loadshifters(ppu* PPU) {
  ...
  attribshiftreg1 =
      (attribshiftreg1 & 0xFF00) | ((next_tile_attr & 0x1) ? 0xFF : 0x00);
  attribshiftreg2 =
      (attribshiftreg2 & 0xFF00) | ((next_tile_attr & 0x2) ? 0xFF : 0x00);
}
...
void ppu_run(ppu* PPU) {
  if ((((PPU->scanline >= 0) && (PPU->scanline < 240)) ||
    ...
    if (((PPU->tick > 0) && (PPU->tick <= 257)) ||
        ((PPU->tick >= 321) && (PPU->tick <= 337))) {
      if (!(PPU->tick == 321 || PPU->tick == 1)) updateshifters(PPU);
      if (PPU->tick != 1) switch ((PPU->tick - 1) % 8) {
          case 0:
            loadshifters(PPU);
            next_tile_id =
                ppu_readb(PPU->ppumap, 0x2000 | (PPU->vramaddr & 0x0FFF));
            break;
          case 2:
            next_tile_attr =
                ppu_readb(PPU->ppumap, 0x23C0 | (PPU->vramaddr & (0x3 << 10)) |
                                           ((PPU->vramaddr & 0x380) >> 4) |
                                           ((PPU->vramaddr & 0x1C) >> 2));
            ...
            break;
          case 4:
            next_pattern_lsb = ppu_readb(
                PPU->ppumap, ((PPU->registers.PPUCTRL & 0x10) << 8) |
                                 (next_tile_id << 4) |
                                 ((PPU->vramaddr & 0x7000) >> 12) + 0);
            break;
          case 6:
            next_pattern_msb = ppu_readb(
                PPU->ppumap, ((PPU->registers.PPUCTRL & 0x10) << 8) |
                                 (next_tile_id << 4) |
                                 ((PPU->vramaddr & 0x7000) >> 12) + 8);
            break;
          case 7:
            incrementx(PPU);
            break;
      ...
      if ((PPU->scanline < 240)) {
        if ((PPU->tick > 0) && (PPU->tick < 257)) {
          ...
          uint8_t bgpixel = 0, bgpalette = 0, fgpixel = 0, fgpalette = 0,
                  fgpriority = 0;
          // doing the Secondary OAM clearing process in one go, everyone
          // recommended so but less accurate
          if (PPU->tick == 1) {
            OAMentry = 0;
            n = m = 0;
            wait  = 0;
            ...
            if (!wait) {
              ...
              if ((diff >= 0) &&
                  (diff < ((PPU->registers.PPUCTRL & (1 << 5)) ? 16 : 8))) {
                if (OAMentry != 8) {
                  memcpy(&(PPU->currOAM[OAMentry]), PPU->OAMpointer + n * 4, 4);
                  OAMentry++;
                  if (n == 0) zerosprflag = 1;
                }
                n    = (n + 1) & 0x3F;
                wait = 7;
              } else {
          ...
          if (SHOWBG) {
            bgpixel = (((patternshiftreg2 & mux) ? 1 : 0) << 1) |
                      ((patternshiftreg1 & mux) ? 1 : 0);
            bgpalette = (((attribshiftreg2 & mux) ? 1 : 0) << 1) |
                        ((attribshiftreg1 & mux) ? 1 : 0);
          }
          if (SHOWCHR) {
            for (int i = 0; i < 8; i++) {
              if (!spritex[i]) {
                fgpixel = (((spritereg2[i] & 0x80) ? 1 : 0) << 1) |
                          ((spritereg1[i] & 0x80) ? 1 : 0);
                if (fgpixel) {
          ...
          PPU->gfx[(PPU->scanline * 256) + (PPU->tick - 1)] =
              getargbfrompal(PPU, pixel, palette);
        }
      }
    }
    ...
    if (PPU->tick == 257) {
      ...
      for (int i = 0; i < 8; i++) {
        if (PPU->currOAM[i].xpos == 0xFF) break;
          ...
          if (PPU->currOAM[i].attr & 0x80) {
            if ((PPU->scanline - PPU->currOAM[i].ypos) < 8) {
              spritepatternaddr = ((PPU->currOAM[i].attr & 0x1) << 12) |
                                  ((PPU->currOAM[i].attr & 0xFE + 1) << 4) |
                                  (7 - diff);
            } else
              spritepatternaddr = ((PPU->currOAM[i].attr & 0x1) << 12) |
                                  ((PPU->currOAM[i].attr & 0xFE) << 4) |
                                  (7 - diff);
          } else {
            if ((PPU->scanline - PPU->currOAM[i].ypos) < 8) {
              spritepatternaddr = ((PPU->currOAM[i].attr & 0x1) << 12) |
        ...
        if (PPU->currOAM[i].attr & 0x40) {
          ...
          spritepattern1 =
              (spritepattern1 & 0xF0) >> 4 | (spritepattern1 & 0x0F) << 4;
          spritepattern1 =
              (spritepattern1 & 0xCC) >> 2 | (spritepattern1 & 0x33) << 2;
          spritepattern1 =
              (spritepattern1 & 0xAA) >> 1 | (spritepattern1 & 0x55) << 1;
          spritepattern2 =
              (spritepattern2 & 0xF0) >> 4 | (spritepattern2 & 0x0F) << 4;
          spritepattern2 =
              (spritepattern2 & 0xCC) >> 2 | (spritepattern2 & 0x33) << 2;
          spritepattern2 =
              (spritepattern2 & 0xAA) >> 1 | (spritepattern2 & 0x55) << 1;
        }
    ...
    if (PPU->tick == 256) incrementy(PPU);
    if (PPU->scanline == 261) {
      if (PPU->tick == 1)
...
uint8_t ppu_read_reg(memmap* map, uint16_t addr) {
  ...
  switch (addr & 0x7) {
    ...
    case 0x3:
      break;
    case 0x4:
      if (VBLANK) return PPU->OAMpointer[PPU->registers.OAMADDR];
      break;
    case 0x5:
      break;
    case 0x6:
