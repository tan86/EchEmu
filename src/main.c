...
int main(int argc, char* argv[]) {
  ...
  window = SDL_CreateWindow("NESEmu", SDL_WINDOWPOS_UNDEFINED,
  ...
  for (;;) {
    ...
    SDL_PumpEvents();
    if (NES->controller1.strobe) poll_input(&(NES->controller1), state);
    if (state[SDL_SCANCODE_ESCAPE]) free_nes(NES);
...
void init_nes(nes* NES, char* game) {
  ...
  registermem(&(NES->CPU.cpumap[2]), 0x20, 0x4000, NES->controlregs,
              CONTROLREG);
  registermem(&(NES->CPU.cpumap[3]), 0x2000, 0x6000,
              NES->cart.batteryrampointer, RAMT);
  registermem(&(NES->CPU.cpumap[4]), (NES->cart.prgromno) * 0x4000, 0x8000,
              NES->cart.prgrompointer, RAMT);

  NES->CPU.PrgCount = cpureadw(NES->CPU.cpumap, NES->CPU.PrgCount);

  /*a pointer to NES structure to reference to other objects, just a nasty hack
   * for now, pardon for this. Can probably be done with consecutive
   * container_ofs to avoid this uglyness*/
  ...
  registermem(&((NES->PPU).ppumap[0]), 0x2000, 0x0000, NES->cart.chrrompointer,
              RAMT);
  if (NES->cart.mir == NOMIR) {
    // introduced some redundancy in here, I didn't thought of it at first ;|
    registermem(&((NES->PPU).ppumap[1]), 0x400, 0x2000, (NES->PPU.nametable),
                RAMT);
    registermem(&((NES->PPU).ppumap[2]), 0x400, 0x2400, (NES->PPU.nametable),
                RAMT);
    registermem(&((NES->PPU).ppumap[3]), 0x400, 0x2800, (NES->PPU.nametable),
                RAMT);
    registermem(&((NES->PPU).ppumap[4]), 0x400, 0x2C00, (NES->PPU.nametable),
                RAMT);
  } else if (NES->cart.mir == VERTICALMIR) {
    registermem(&((NES->PPU).ppumap[1]), 0x400, 0x2000, (NES->PPU.nametable),
                RAMT);
    registermem(&((NES->PPU).ppumap[2]), 0x400, 0x2400,
                (NES->PPU.nametable + 4 * 16 * 16), RAMT);
    registermem(&((NES->PPU).ppumap[3]), 0x400, 0x2800, (NES->PPU.nametable),
                RAMT);
    registermem(&((NES->PPU).ppumap[4]), 0x400, 0x2C00,
                (NES->PPU.nametable + 4 * 16 * 16), RAMT);
    ...
    registermem(&((NES->PPU).ppumap[2]), 0x400, 0x2400, (NES->PPU.nametable),
                RAMT);
    registermem(&((NES->PPU).ppumap[3]), 0x400, 0x2800,
                (NES->PPU.nametable + 8 * 16 * 16), RAMT);
    registermem(&((NES->PPU).ppumap[4]), 0x400, 0x2C00,
                (NES->PPU.nametable + 8 * 16 * 16), RAMT);
  }
